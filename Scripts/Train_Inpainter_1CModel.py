...
configFile = "Configs/Ice_1C_NN_Simple.json"
...
trainset = NNs.OneChannelDataset(f'{dataPath}/Train')#, transform=transform_train)
testset = NNs.OneChannelDataset(f'{dataPath}/Test')#, transform=transform_test)
...
for epoch in range(num_epochs):
    ...
    for i, (maskedChemical, groundTruth, mask, batchLabels) in enumerate(trainloader):
        # Zero gradients and make forward pass
        optimizer.zero_grad()
        outputs = model(maskedChemical)

        # Calculate losses
        loss = criterion(outputs, groundTruth)
        similarity = Utils.calculate_ssim(outputs[:, 0, :, :], groundTruth[:, 0, :, :])
    ...
    with torch.no_grad():
        for maskedChemical, groundTruth, mask, batchLabels in testloader:
            outputs = model(maskedChemical)
    ...
    if (epoch+1) % 5 == 0:
        # Plot example data
        maskedChemical, groundTruth, mask, sampleLabel = next(iter(testloader))
        with torch.no_grad():
            predicted = model(maskedChemical)
        Utils.plot_Test_1C(predicted, maskedChemical, groundTruth,
                  mask, save = f'{plotsFolder}/Epoch_{epoch}')
